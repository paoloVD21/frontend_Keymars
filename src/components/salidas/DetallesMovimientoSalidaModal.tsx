...
export const DetallesMovimientoSalidaModal: React.FC<DetallesMovimientoSalidaModalProps> = ({
    ...
    const renderProductosTable = () => {
        if (!detallesMovimiento?.detalles || detallesMovimiento.detalles.length === 0) {
            return null;
        }

        const total = detallesMovimiento.detalles.reduce((sum, detalle) => sum + Number(detalle.precio_total), 0);
